Database required for assignment is vet_serv.db

Instructions referenced in document in M5pro_Instructions.docx

Important (Requirements):

1. Exception handling must be considered

2. Program must be modularized (Functions created and code reference in 2 files).

3. CSV files created must be submitted with code (should be a total of 3 csv files).

3. Submit a zipped folder containing the finished code solution file(s), the csv files and the vet_serv.db through the assignment link below (M5Pro submission link)

# Grade: 70
" - H. Seidi"